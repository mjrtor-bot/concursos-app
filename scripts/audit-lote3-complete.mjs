...
import { formatarQuestao } from "./batch3_modules/helper.mjs";
...
async function auditLote3() {
  console.log("=== INICIANDO AUDITORIA COMPLETA DO LOTE 3 ===");
  ...
  console.log(`[+] Total de questões em código (Lote 3): ${totalRaw} (Esperado: 500)`);
  ...
  console.log(`[+] IDs únicos no Lote 3: ${idsSet.size}/${todasQuestoes.length}`);
  console.log(`[+] Fingerprints únicos no Lote 3: ${fpsSet.size}/${todasQuestoes.length}`);

  if (idsSet.size !== 500 || fpsSet.size !== 500) {
    throw new Error(`Colisão interna de ID ou Fingerprint no Lote 3!`);
  }
  ...
  console.log("\n[>] Calculando matriz de similaridade textual (Jaccard 3-shingles) entre enunciados do Lote 3...");
  ...
  if (pares95 > 0) {
    console.warn(`[AVISO] Detectados ${pares95} pares com similaridade >= 95%.`);
  }
  ...
  fs.writeFileSync(
    path.resolve(process.cwd(), "scripts/audit_lote3_results.json"),
    JSON.stringify(relatorio, null, 2),
    "utf8"
  );

  console.log("\n=========================================================");
  console.log("   AUDITORIA DO LOTE 3 CONCLUÍDA E SALVA COM SUCESSO!    ");
  console.log("=========================================================");
}

auditLote3().catch(err => {
  console.error("ERRO NA AUDITORIA DO LOTE 3:", err);
  process.exit(1);
});
