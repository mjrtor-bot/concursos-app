...
console.log("=== INICIANDO VALIDAÇÃO ESTRUTURAL DO LOTE 8 ===");
console.log(`Total de questões carregadas: ${TODAS_QUESTOES_LOTE8.length}`);

if (TODAS_QUESTOES_LOTE8.length !== 500) {
  console.error(`ERRO: Quantidade de questões diferente de 500! Atual: ${TODAS_QUESTOES_LOTE8.length}`);
  process.exit(1);
}
...
for (let i = 0; i < TODAS_QUESTOES_LOTE8.length; i++) {
  const q = TODAS_QUESTOES_LOTE8[i];
...
console.log("\n--- ESTATÍSTICAS ESTRUTURAIS DO LOTE 8 ---");
console.log(`Questões: ${TODAS_QUESTOES_LOTE8.length}`);
...
if (erros > 0) {
  ...
  console.log("\n✅ VALIDAÇÃO ESTRUTURAL APROVADA COM SUCESSO (0 ERROS)!");
}
