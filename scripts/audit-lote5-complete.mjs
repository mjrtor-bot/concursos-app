...
async function auditLote5() {
  ...
  console.log(`[+] Total de questões em código (Lote 5): ${totalRaw} (Esperado: 500)`);
  ...
  console.log(`[+] IDs únicos no Lote 5: ${idsSet.size}/${todasQuestoes.length}`);
  console.log(`[+] Fingerprints únicos no Lote 5: ${fpsSet.size}/${todasQuestoes.length}`);

  if (idsSet.size !== 500 || fpsSet.size !== 500) {
    throw new Error(`Colisão interna de ID ou Fingerprint no Lote 5!`);
  }
  ...
  console.log("\n[>] Calculando matriz de similaridade textual (Jaccard 3-shingles) entre enunciados do Lote 5...");
  ...
  fs.writeFileSync(
    path.resolve(process.cwd(), "scripts/audit_lote5_results.json"),
    JSON.stringify(relatorio, null, 2),
    "utf8"
  );

  console.log("\n=========================================================");
  console.log("   AUDITORIA DO LOTE 5 CONCLUÍDA E SALVA COM SUCESSO!    ");
  console.log("=========================================================");
}

auditLote5().catch(err => {
  console.error("ERRO NA AUDITORIA DO LOTE 5:", err);
  process.exit(1);
});
