...
console.log("=== Verificando Módulos do Lote 1 ===");
const modulos = [
...
for (const mod of modulos) {
  ...
  for (const raw of mod.list) {
    ...
    if (fpSet.has(formatted.questao.fingerprint_hash)) {
      console.warn(`[WARN] Fingerprint duplicado: ${formatted.questao.fingerprint_hash} (slug: ${raw.idSlug})`);
    }
...
console.log("\n==========================================");
console.log(`TOTAL DE QUESTÕES VERIFICADAS: ${totalQuestoes} (Esperado: 500)`);
console.log(`TOTAL DE ALTERNATIVAS GERADAS: ${totalAlts}`);
console.log(`IDs ÚNICOS: ${idsSet.size}`);
console.log(`FINGERPRINTS ÚNICOS: ${fpSet.size}`);
console.log("==========================================");
