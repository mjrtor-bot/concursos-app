...
import { TODAS_QUESTOES_LOTE8, prepararParaBanco } from "./batch8_modules/index.mjs";
...
async function importLote8() {
  console.log("=========================================================");
  console.log("      INICIANDO INGESTÃO DO LOTE 8 NO SUPABASE PROD      ");
  console.log("=========================================================\n");
  ...
  if (initialQCount !== 4760) {
    throw new Error(`Contagem inicial no banco inesperada: esperado 4.760 questões, obtido ${initialQCount}`);
  }
  ...
  if (todasQuestoes.length !== 500) {
    throw new Error(`Total de questões do Lote 8 deve ser 500. Encontrado: ${todasQuestoes.length}`);
  }
  if (todasAlternativas.length !== 1900) {
    throw new Error(`Total de alternativas do Lote 8 deve ser 1.900. Encontrado: ${todasAlternativas.length}`);
  }

  console.log("\n[>] Inserindo 500 questões via chunked upsert (chunk = 50)...");
  await chunkUpsert("questoes", todasQuestoes, 50);
  console.log("[✓] Todas as 500 questões do Lote 8 inseridas/atualizadas com sucesso!");
  ...
  console.log(`[✓] Todas as ${todasAlternativas.length} alternativas do Lote 8 inseridas/atualizadas com sucesso!`);
  ...
  console.log(`[+] Questões no Supabase: ${finalQCount} (Esperado: 5.260 | Delta: +${finalQCount - initialQCount})`);
  console.log(`[+] Alternativas no Supabase: ${finalACount} (Esperado: 20.312 | Delta: +${finalACount - initialACount})`);

  if (finalQCount !== 5260) {
    throw new Error(`ERRO: Contagem final de questões no banco diferente de 5.260! Atual: ${finalQCount}`);
  }
  if (finalACount !== 20312) {
    throw new Error(`ERRO: Contagem final de alternativas no banco diferente de 20.312! Atual: ${finalACount}`);
  }

  console.log("\n=========================================================");
  console.log("  [SUCESSO] INGESTÃO DO LOTE 8 CONCLUÍDA COM 100% ÊXITO! ");
  console.log("=========================================================");
}

importLote8().catch((err) => {
  console.error("Erro fatal durante a importação do Lote 8:", err);
  process.exit(1);
});
