...
export function formatarQuestao(raw, indexGlobal) {
  const qId = stringToUuid(`q-pol-batch6-${raw.idSlug || indexGlobal}`);
  ...
  const alternativas = raw.alternativas.map((alt, idx) => ({
    id: stringToUuid(`alt-pol-batch6-${raw.idSlug || indexGlobal}-${idx}`),
    questao_id: qId,
...
export function validateQuestionBatch(rawQuestions) {
  ...
  for (let i = 0; i < rawQuestions.length; i++) {
    ...
    try {
      ...
      const uuid = stringToUuid(`q-pol-batch6-${raw.id || raw.idSlug || i + 1}`);
      validQuestions.push({
        ...raw,
        id: id,
        uuid: uuid,
